      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-300 mt-2">Comprehensive platform performance metrics</p>
          </div>
          <div className="flex gap-3">
            <select
              value={timeRange}
              onChange={(e) => setTimeRange(e.target.value)}
              className="px-4 py-2 border border-white/30 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat) => {
            ...
            return (
              <div key={stat.label} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg shadow-sm border border-white/20 p-6">
        ...
        {/* Performance Metrics */}
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg shadow-sm border border-white/20 p-6 mb-8">
          <h2 className="text-xl font-semibold text-white mb-6">Performance Metrics</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {performanceMetrics.map((metric) => (
              <div key={metric.label} className="border border-white/20 rounded-lg p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg shadow-sm border border-white/20 p-6">
            <h2 className="text-xl font-semibold text-white mb-6">Revenue by Type</h2>
          ...
          {/* Top Drivers */}
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg shadow-sm border border-white/20 p-6">
            <h2 className="text-xl font-semibold text-white mb-6">Top Performing Drivers</h2>
            
            <div className="space-y-4">
              {topDrivers.map((driver, index) => (
                <div key={driver.name} className="flex items-center gap-4 pb-4 border-b border-white/20 last:border-0">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/20">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-200">Rank</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-200">Company</th>
                  <th className="text-right py-3 px-4 text-sm font-semibold text-gray-200">Shipments</th>
                  <th className="text-right py-3 px-4 text-sm font-semibold text-gray-200">Total Spent</th>
                </tr>
              </thead>
              <tbody>
                {topShippers.map((shipper, index) => (
                  <tr key={shipper.name} className="border-b border-gray-100 hover:bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900">
                    ...
                    <td className="py-3 px-4 font-medium text-white">{shipper.name}</td>
                    <td className="py-3 px-4 text-right text-gray-200">{shipper.shipments}</td>
